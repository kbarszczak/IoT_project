...
int ISM43362::setCredentialsForAP(const char *ssid, const char *pass, nsapi_security_t security) {
    if (!(_parser.send("AS=0,%s", ssid) && check_response())) {
        debug_if(_ism_debug, "\tSetting SSID failed\r\n");
        return 1;
    }
    debug_if(_ism_debug, "\tSetting SSID success\r\n");

    if(security != NSAPI_SECURITY_NONE) {
        if (!(_parser.send("A1=4") && check_response())) {
            debug_if(_ism_debug, "\tSetting WPA2/WPA wifi failed\r\n");
            return 1;
        }

        if (!(_parser.send("A2=%s", pass) && check_response())) {
            debug_if(_ism_debug, "\tSetting WPA2 key failed\r\n");
            return 1;
        }
        debug_if(_ism_debug, "\tWPA2 key set success\r\n");
    }

    return 0;
}

int ISM43362::startSoftAp() {
    if (!(_parser.send("A0"))) {
        debug_if(_ism_debug, "\tApen AP and DHCP failed\r\n");
        return 1;
    }
    debug_if(_ism_debug, "\tSoftAP started\r\n");

    return 0;
}

int ISM43362::listenForConnection(){
    char tmp[250];
    while (true) {
        _parser.recv("%[^\n]\n", tmp);
        if (strstr(tmp, "JOIN")) {
